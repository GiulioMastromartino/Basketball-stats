...
from core.models import PlayerStat, Game, db
from core.utils import (
    calculate_possessions, calculate_efficiency, calculate_ortg, 
    calculate_ppp, calculate_ts_percent, calculate_efg_percent,
    calculate_two_point_stats, parse_minutes, FT_ATTEMPT_WEIGHT,
    THREE_POINT_WEIGHT, get_player_stats_averages, normalize_per_100_possessions,
    safe_percentage
)
...
class AnalyticsService:
    ...
    def calculate_game_stats(stats):
        ...
        for s in stats:
            ...
            # Game Score
            s.game_score = (
                s.points + 0.4 * s.fgm - 0.7 * s.fga - 0.4 * (s.fta - s.ftm) +
    ...
    def calculate_team_averages(game_ids, db_session=None):
        ...
        if not team_stats or not team_stats.player_games:
            return {'ppg': 0, 'rpg': 0, 'apg': 0, 'ts_pct': 0, 'efg_pct': 0, 'ast_tov': 0, 'ortg': 0}

        ts_pct = calculate_ts_percent(team_stats.total_pts, team_stats.total_fga, team_stats.total_fta)
        efg_pct = calculate_efg_percent(team_stats.total_fgm, team_stats.total_tpm, team_stats.total_fga)
        ...
        all_stats = (
            session.query(PlayerStat.fga, PlayerStat.fta, PlayerStat.oreb, PlayerStat.tov)
            .filter(PlayerStat.game_id.in_(game_ids))
            .filter(PlayerStat.minutes != "00:00")
            .filter(PlayerStat.minutes != "0")
            .all()
        )
        total_possessions = sum(calculate_possessions(s.fga, s.fta, s.oreb, s.tov) for s in all_stats)
    ...
    def calculate_team_rankings(player_name, game_ids, report_data, db_session=None):
        ...
        for name, group in groupby(raw_stats, key=lambda x: x[0]):
            poss = sum(calculate_possessions(s.fga, s.fta, s.oreb, s.tov) for s in group)
        ...
        for metric in ['ppg', 'rpg', 'apg', 'ts_pct', 'efg_pct', 'ast_tov', 'ortg']:
            ...
            rank = next((i for i, p in enumerate(sorted_players, 1) if p['name'] == player_name), None)
    ...
    def calculate_player_metrics(stats, game_map, games_played):
        ...
        if live_game_stats:
            avg_plus_minus = sum(s.plus_minus for s in live_game_stats) / len(live_game_stats)
        else:
            avg_plus_minus = 0
        ...
        shooting_data = {
            ...
            'two_pt': {'made': round(two_pt_made, 1), 'att': round(two_pt_att, 1), 'pct': safe_percentage(two_pt_made, two_pt_att)}
        }
        ...
        for s in stats:
            ...
            gmsc = (s.points + 0.4*s.fgm - 0.7*s.fga - 0.4*(s.fta-s.ftm) + 
    ...
    def calculate_enhanced_team_metrics(games, game_ids, db_session=None):
        ...
        return {
            ...
            "games": games,
            "top_contributors": AnalyticsService.get_top_contributors(game_ids, db_session),
            "opponent_stats": AnalyticsService.analyze_opponents(games),
            "team_shooting": AnalyticsService.calculate_team_shooting(game_ids, db_session),
            "plus_minus_leaders": AnalyticsService.calculate_plus_minus_leaders(games, db_session)
        }
    ...
    def get_top_contributors(game_ids, db_session=None):
        ...
        return {
            'points': [{'player': p.player_name, 'total': int(p.total), 'avg': round(p.avg, 1), 'games': p.games} for p in pts],
            'rebounds': [{'player': p.player_name, 'total': int(p.total), 'avg': round(p.avg, 1)} for p in reb],
            'assists': [{'player': p.player_name, 'total': int(p.total), 'avg': round(p.avg, 1)} for p in ast]
        }

    @staticmethod
    def analyze_opponents(games):
        """Analyze performance against different opponents"""
        stats = defaultdict(lambda: {'wins': 0, 'losses': 0, 'pf': 0, 'pa': 0, 'games': 0})
        for g in games:
            opp = g.opponent
            stats[opp]['games'] += 1
            stats[opp]['pf'] += g.team_score
            stats[opp]['pa'] += g.opponent_score
            if g.result == 'W': stats[opp]['wins'] += 1
            else: stats[opp]['losses'] += 1
