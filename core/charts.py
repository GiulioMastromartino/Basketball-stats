...
def _create_court_plot(shots, is_team=False):
    ...
    try:
        ...
        ft_circle = patches.Circle(
            (250, 195.3), 60, linewidth=line_width, edgecolor="black", facecolor="none"
        )
        ...
        # Hoop
        hoop = patches.Circle(
            (250, 52.5), 7.5, linewidth=line_width, edgecolor="black", facecolor="none"
        )
        ax.add_patch(hoop)
        ...
        if makes:
            ...
            ax.scatter(
                ...
                make_y,
                c="#28a745",
                s=60 if is_team else 80,
                alpha=0.5 if is_team else 0.6,
        ...
        if is_team:
            ...
            three_pct = (
                (len(three_pt_makes) / len(three_pt_shots) * 100)
                if three_pt_shots
                else 0
            )
            ...
            two_pct = (
                (len(two_pt_makes) / len(two_pt_shots) * 100) if two_pt_shots else 0
            )

            stats_text = f"Overall: {total_makes}/{total_shots} ({fg_pct:.1f}%)\n"
            stats_text += (
                f"2PT: {len(two_pt_makes)}/{len(two_pt_shots)} ({two_pct:.1f}%)\n"
            )
            stats_text += (
                f"3PT: {len(three_pt_makes)}/{len(three_pt_shots)} ({three_pct:.1f}%)"
            )
...
def generate_player_charts(stats, game_map, player_name, db_session=None):
    ...
    try:
        ...
        if len(points) >= 3:
            ma3 = [
                sum(points[i - 2 : i + 1]) / 3 if i >= 2 else points[i]
