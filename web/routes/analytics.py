...
def game_summary_pdf(game_id):
    """Generate full game summary PDF with box score, stats, and analysis - OPTIMIZED"""
    ...
    # === OPTIMIZATION: Bulk-fetch ALL shot events for the game ===
    all_shots = ShotEvent.query.filter_by(game_id=game_id).all()
    
    # Build shots_by_player dict
    shots_by_player = defaultdict(list)
    for shot in all_shots:
        if shot.x_loc is None or shot.y_loc is None:
            continue
        shots_by_player[shot.player_name].append({
            'x': shot.x_loc, 
            'y': shot.y_loc, 
            'result': shot.result
        })
    
    # === OPTIMIZATION: Bulk-aggregate play chips in ONE query ===
    play_chip_rows = (
        db.session.query(
        ...
        .outerjoin(Play, Play.id == ShotEvent.play_id)
        .filter(ShotEvent.game_id == game_id)
        .group_by(ShotEvent.player_name, Play.name)
        .all()
    )
    
    # Build chips_by_player dict
    chips_by_player = defaultdict(list)
    for player_name, play_name, pts in play_chip_rows:
        if not pts or pts <= 0:
            continue
        label = play_name or "Unassigned"
        chips_by_player[player_name].append({'label': label, 'pts': int(pts)})
    
    # === NOW assign per-player data WITHOUT extra queries ===
    for s in stats_with_metrics:
        # Shots
        s.shots = shots_by_player.get(s.player_name, [])
        s.shot_chart_svg = len(s.shots) > 0
        
        # Play chips
        chips = chips_by_player.get(s.player_name, [])
    ...
    shot_chart = _generate_team_shot_chart([game_id]) if all_shots else ""
...
# ... rest of the file continues with team_report_pdf, player_report_pdf, download_all_reports and helper functions ...
# (file is too long - rest omitted for brevity, just showing the fixed game_summary_pdf function)
