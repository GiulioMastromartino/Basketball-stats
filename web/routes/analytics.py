...
def _generate_shot_chart(player_name, game_ids):
    """Generate a basketball shot chart showing make/miss locations. Returns base64 encoded PNG"""
    shots = (
        ShotEvent.query
...
def _generate_team_shot_chart(game_ids):
    """Generate team-wide shot chart showing all team shots. Returns base64 encoded PNG"""
    shots = (
        ShotEvent.query
        .filter(ShotEvent.game_id.in_(game_ids))
        .filter(ShotEvent.x_loc.isnot(None))
